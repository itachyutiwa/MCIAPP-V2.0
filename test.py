...
def createDirs():
    ...
    if str(COUTIER_entry.get()) != "":
        # Créer les dossiers dossier_production et dossier_consommation à l'intérieur de dossier_coutier_pays
        dossier_production_path = os.path.join(base_directory, getCourtier() , "PRODUCTION")
        if not os.path.exists(dossier_production_path):
            os.makedirs(dossier_production_path)

        dossier_consommation_path = os.path.join(base_directory, getCourtier() , "CONSOMMATION")
        if not os.path.exists(dossier_consommation_path):
            os.makedirs(dossier_consommation_path)


        dossier_resultats_path = os.path.join(base_directory, getCourtier() , "RESULTATS")
...
def critere_benef(file, ante=pd.to_datetime(str(start_date_entry.get()), dayfirst=True), post=pd.to_datetime(str(end_date_entry.get())), dayfirst=True):
    ...
    if len(benef_sans_doublon) != 0:
        ...
        path_sans_doublon = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "SANS_DOUBLONS_BENEF.xlsx")
        benef_sans_doublon.to_excel(path_sans_doublon,sheet_name="SANS_DOUBLONS_BENEF")

    if len(benef_doublon) != 0:
        path_doublon = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "DOUBLONS_BENEF.xlsx")
        benef_doublon.to_excel(path_doublon,sheet_name="DOUBLONS_BENEF")

    if len(police_benef) != 0:
        path_police_benef = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "POLICE_BENEF.xlsx")
        police_benef.to_excel(path_police_benef,sheet_name="POLICE_BENEF")

    if len(ENFANT_SUP_25) != 0:
        path_ENFANT_SUP_25 = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "ENFANTS_SUP_25_ANS.xlsx")
        ENFANT_SUP_25.to_excel(path_ENFANT_SUP_25,sheet_name="ENFANTS_SUP_25_ANS")

    if len(ADULTES_SUP_60) != 0:
        path_ADULTES_SUP_60 = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "ADULTES_SUP_60_ANS.xlsx")
        ADULTES_SUP_60.to_excel(path_ADULTES_SUP_60,sheet_name="ADULTES_SUP_60_ANS")

    if len(benef_sans_date_naiss) != 0:
        path_benef_sans_date_naiss = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "BENEF_SANS_DATE_NAISS.xlsx")
        benef_sans_date_naiss.to_excel(path_benef_sans_date_naiss,sheet_name="BENEF_SANS_DATE_NAISS")

   
    if len(df_erreur_age) != 0:
        path_df_erreur_age = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "BENEF_ERREUR_DATE_NAISS.xlsx")
        df_erreur_age[df_erreur_age["numPolice"].isin(liste_errs)].to_excel(path_df_erreur_age,sheet_name="BENEF_ERREUR_DATE_NAISS")
    ...
    if len(df_production) != 0:
        path_df_production = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/RESULTATS", "PRODUCTION.xlsx")
...
def critere_emission(file, post=pd.to_datetime(str(end_date_entry.get()), dayfirst=True)):
    ...
    if len(doublons_emission) != 0:
        path_doublon = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "DOUBLONS_COTISATION.xlsx")
        doublons_emission.to_excel(path_doublon, sheet_name="DOUBLONS_COTISATION")

    if len(sans_doublons_emission) != 0:
        path_sans_doublon = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "SANS_DOUBLONS_COTISATION.xlsx")
        sans_doublons_emission.to_excel(path_sans_doublon, sheet_name="SANS_DOUBLONS_COTISATION")

    if len(police_emission) != 0:
        path_police_emission = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/PRODUCTION", "POLICE_COTISATION.xlsx")
...
def critere_conso(file):
    ...
    if len(doublons_conso) != 0:
        path_doublon = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/CONSOMMATION", "DOUBLONS_CONSO.xlsx")
        doublons_conso.to_excel(path_doublon,sheet_name="DOUBLONS_CONSO")

    if len(sans_doublons_conso) != 0:
        path_sans_doublons_conso = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/CONSOMMATION", "SANS_DOUBLONS_CONSO.xlsx")
        sans_doublons_conso.to_excel(path_sans_doublons_conso,sheet_name="SANS_DOUBLONS_CONSO")

    if len(police_conso) != 0:
        path_police_conso = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/CONSOMMATION", "POLICE_CONSO.xlsx")
    ...
    if (len(rs["result"]) == len(rs["anorm"]) == 6):
        ...
        if len(df_prestation) != 0:
            path_df_prestation = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/RESULTATS", "PRESTATION.xlsx")
...
def resume(file_benef, file_emission, file_conso, seuil_ctx, ante=pd.to_datetime(str(start_date_entry.get())), post=pd.to_datetime(str(end_date_entry.get()))):
    ...
    if len(ENFANT_SUP_25_CONSO) != 0:
        #path_ENFANT_SUP_25_CONSO = os.path.join(dossier_consommation_path, "ENFANT_SUP_25_CONSO.xlsx")
        pass
        try: 
    ...
    if len(ADULTES_SUP_60_CONSO) != 0:
        #path_ADULTES_SUP_60_CONSO = os.path.join(dossier_consommation_path, "ADULTE_SUP_60_CONSO.xlsx")
        pass
        try:
    ...
    if len(CONSO_COUTEUSES) != 0:
        path_CONSO_COUTEUSES = os.path.join(f"CONTROLE GESTION DELEGUE/{str(COUTIER_entry.get())}/CONSOMMATION", "CONSO_COUTEUSES.xlsx")
        CONSO_COUTEUSES.to_excel(path_CONSO_COUTEUSES)

    if len(benef_sans_date_naiss) != 0:
        #path_benef_sans_date_naiss = os.path.join(dossier_production_path, "BENEF SANS DATE DE NAISSANCE.xlsx")
...
def start_progress():
    ...
    critere_benef(selected_beneficiaire_path, pd.to_datetime(str(start_date_entry.get()), dayfirst=True), pd.to_datetime(str(end_date_entry.get()), dayfirst=True))
    critere_emission(selected_emission_path, pd.to_datetime(str(end_date_entry.get()), dayfirst=True))
    critere_conso(selected_consommation_path)
    resume(selected_beneficiaire_path, selected_emission_path, selected_consommation_path, seuil_entry.get(), ante=pd.to_datetime(str(start_date_entry.get()), dayfirst=True), post=pd.to_datetime(str(end_date_entry.get()), dayfirst=True))
    update_progress()
    return
